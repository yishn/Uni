...
where $S$ ranges over all finite subsets of $K$. Let $v,w\in\operatorname{Spv}(K)$. We have:
...
Now $(K_0)_{w_1}$ is henselian both with respect to $w_1$ and with respect to the extension of $w_0$. A contradiction to $\operatorname{cd}(G_{(K_0)_{w_1}})\leq 2$.
...
\begin{lemma}
$\sigma$ induces a bijection $\sigma:\operatorname{Spv}_\text{rd}^{(2)}(K) \to\operatorname{Spv}_\text{rd}^{(2)}(K')$.
\end{lemma}
...
Finally for $T=\{P_1,\ldots,P_r\}\subset U_0$, the map $r_{S,T}$ is injective.
\end{proof}
...
$\varphi^{S,T}$ actually independent of $T$, thus we can omit the $T$ from its notation. Now if $T$ ranges over all $S$-admissible subsets of $X_0$, then $(\star)$ shows:
\[ r_S\circ\varphi^S = \Big(\prod_{P\in X_0 -  S}\varphi_P \Big)\circ r_{S'} \]
In other words, we can replace $T$ by $X_0 -  S$ in $(\star)$. 

We have $\varphi^{S_1}=\varphi^{S_2}|_{K'^{S'_1}}$. Thus, we can combine the isomorphisms $\varphi^S$, where $S$ ranges over the admissible subsets of $X_0$, into an isomorphism $\varphi = \varinjlim \varphi^S: K'^\times \to K^\times$. 
\end{proof}
