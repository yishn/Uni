...
\paragraph{Definition 13.6.}\label{13.6} Ein Divisor $K\in\operatorname{Div}(X)$ heißt \textit{kanonischer Divisor}\index{Divisor!kanonisch}, falls $K\sim D$ mit $\mathcal{L}(D)\cong\omega_X$.

\paragraph{Definition.} Sei $X$ ein projektives Schema über einem Körper $k$ und $\mathcal{F}\in\mathbf{Coh}(X)$. Die \textit{Euler-Charakteristik}\index{Euler-Charakteristik} von $\mathcal{F}$ ist definiert als:
\[\chi(\mathcal{F})=\sum_{i\geq 0}(-1)^i\dim_k\mathrm{H}^i(X,\mathcal{F}) \]

\paragraph{Satz 13.7.}\label{13.7} \textit{(Riemann-Roch)} Sei $X$ eine Kurve vom Geschlecht $g$ und $D\in\operatorname{Div}(X)$. Dann gilt für einen kanonischen Divisor $K$:
