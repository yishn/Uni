...
\[\xymatrix{0\ar[r] & F(U)\ar[r]_-{\operatorname{res}} & \displaystyle\prod_i F(U_i)\ar@<.5ex>[r]\ar@<-.5ex>[r]_-{\operatorname{res}} & \displaystyle\prod_{i,j}F(U_i\cap U_j)} \]
...
\[\xymatrix{
F(U)\ar[rr]^{\varphi(U)}\ar[d]_{\operatorname{res}^U_V} && G(U)\ar[d]^{\operatorname{res}^U_V}\\
F(V)\ar[rr]_{\varphi(V)} && G(V)
} \]
...
\[\xymatrix{ 
F\ar[rr]^-\theta\ar[d]_\phi && F^+\ar@{-->}[dll]^\psi\\
G
} \]
...
\[\xymatrix{\cdots \ar[r] & F^i\ar[r]^{\varphi^i} & F^{i+1}\ar[r]^{\varphi^{i+1}} & F^{i+2}\ar[r] & \cdots } \]
...
\[\xymatrix{
F_i\ar[rr]^{\varphi_i}\ar[d]_{\psi_i} & & \varinjlim F_i\ar@{-->}[lld]^\psi\\
G
} \]
...
\[ \xymatrix{
f^{-1}(V) \ar[r]^-f\ar@{-}[d] & V\ar[r]^-s\ar@{-}[d] & A_{\varphi^{-1}(\mathfrak{p})}\ar[r]^-{\varphi_\mathfrak{p}}\ar@{-}[d] & B_\mathfrak{p}\ar@{-}[d]\\
\mathfrak{p}\ar@{|->}[r] & f(\mathfrak{p}) = \varphi^{-1}(\mathfrak{p})\ar@{|->}[r] & s(\varphi^{-1}(\mathfrak{p}))\ar@{|->}[r] & \varphi_\mathfrak{p}(s(\varphi^{-1}(\mathfrak{p})))
} \]
...
(0)\ar@{|->}[r]\ar@{-}[u] & t_1\quad t_0 \ar@{-}@<-1.8ex>[u]\ar@{-}@<2.2ex>[u]& \ar@{|->}[l] (0)\ar@{-}[u]
} \]
...
\[\xymatrix{
A\ar[d]_{\alpha(f)}\ar[rr]^-{\alpha(f_\nu)} && \Gamma(X_\nu,\mathcal{O}_{X_\nu})\\
\Gamma(X,\mathcal{O}_X)\ar[rru]_-{\operatorname{res}}
} \]
...
A\ar[ur]^{h_\nu} \ar[dr]_{h_\mu} & & \Gamma(X_\nu\cap X_\mu,\mathcal{O}_X)\\
& \Gamma(X_\mu,\mathcal{O}_X)\ar[ur]_{\operatorname{res}} &
} \]
...
\[ \xymatrix{
X\times_S Y\ar[r]^-{p_2}\ar[d]_-{p_1} & Y\ar[d]\\
X\ar[r] & S
}\]
\item Ist $Z$ ein $S$-Schema und $f:Z\to X,\ g:Z\to Y$ Morphismen derart, dass das folgende Diagramm kommutiert:
\[\xymatrix{
Z\ar@/^1pc/[rrd]^g\ar@/_1pc/[ddr]_f\ar@{-->}[rd]^\theta &&\\
&X\times_S Y\ar[r]_-{p_2}\ar[d]^-{p_1} & Y\ar[d]\\
&X\ar[r] & S
} \]
...
\[\xymatrix{
 ...
 R\ar[r]\ar[u] & B\ar[u]_{\tilde{p}_2}
} \]
...
A\ar[r]_-{\tilde{p}_2}\ar@/^1pc/[rru]^{\tilde{f}}& A\otimes_RB\ar@{-->}[ru]_{\tilde{\theta}} &\\
R\ar[r]\ar[u] & B\ar[u]^{\tilde{p}_2}\ar@/_1pc/[ruu]_{\tilde{g}}
} \]
...
\[\xymatrix{
p_1^{-1}(U)\ar[r]^-{p_2}\ar[d]_{p_1} & Y\ar[d]\\
U\ar[r] & S
} \]
...
\[\xymatrix{
X_j\times_S Y\ar[r]^-{p_{1j}}\ar[d]_{p_{2j}} & X_j\ar[d]\\
Y\ar[r] & S
}\qquad\qquad \xymatrix{
Z\ar[d]_{p_2}\ar[r]^-{p_1} & X\ar[d]\\
Y\ar[r] & S
}\]
...
\[\xymatrix{
X_i\times_SY\ar@{^{(}->}[r]\ar[d]_{p_{1i}} & Z\ar[d]^{p_1}\\
X_i\ar@{^{(}->}[r] & X
} \]
...
\[\xymatrix{
Z\ar@/_1pc/[ddr]_f\ar@/^1pc/[rrd]_g\ar@/^1pc/[rrrd]^g\ar@{-->}[rd]_\theta\\
& X_i\times_{S_i}Y_i\ar[r]_-{p_2}\ar[d]^{p_1} & Y_i\ar@{^{(}->}[r]\ar[d] & Y\ar[dd]^r\\
& X_i\ar@{^{(}->}[d]\ar[r] & S_i\ar@{^{(}->}[dr] &\\
& X\ar[rr]_q & & S
} \]
...
\[ \xymatrix{
Z\ar@/^1pc/[rrd]^\varphi\ar@{-->}[rd]_\theta\ar@/_1pc/[ddr]_\psi\\
& X\ar[r]\ar[d]\times_SY & X\ar[d]_\xi\ar@/^1pc/[ddr] &\\
& Y\ar[r]^\eta\ar@/_1pc/[rrd] & S\ar[dr]^\sigma&\\
&&&T
}\]
...
\[\xymatrix{
&X_1 \ar[r]^f& X_2\\
X_1\times_SY_1\ar[dr]_{p_2}\ar[ur]^{p_1}\ar@{-->}[rrr]^{f\times_Sg} &&& X_2\times_SY_2\ar[lu]_{q_1}\ar[ld]^{q_2}\\
& Y_1\ar[r]_g&Y_2
} \]
...
\[\xymatrix{
X_y\ar[r]^-{p_2}\ar[d]_{p_1} & \operatorname{Spec}\kappa(y)\ar[d]^i\\
X\ar[r]_f & Y
}\]
...
\mathcal{O}_{Z,x_0} \ar[r]\ar[u]& \mathcal{O}_{T,t_0}=R\ar@{^{(}->}[u]\\
\mathfrak{m}_{Z,x_0}\ar[r]\ar@{^{(}->}[u] & \mathfrak{m}_{T,t_0}\ar@{^{(}->}[u]
}\]
...
\[\xymatrix{
U\ar[d]_i\ar[r] & X\ar[d]^f\\
T\ar[r]\ar@<.5ex>[ur]^{h_1}\ar@<-.5ex>[ur]_{h_2} & Y
} \]
...
\[\xymatrix{
U\ar[r]\ar[d]^i\ar@/_3pc/[rdd]_{h_2i}\ar@/^3pc/[rrd]^{h_1i} & X\ar[d]^-\Delta &\\
T\ar[r]_-{h''} & X\times_Y X\ar[r]^-{p_1}\ar[d]_-{p_2} & X\ar[d]^f\\
& X\ar[r]_f & Y
} \]
...
\[\xymatrix{
\operatorname{Spec}(K)\ar[r]^-i & T\ar[dr]^h\ar@/^1pc/[rrd]^{p_1h}\ar@/_1pc/[ddr]_{p_2h}\\
& & X\times_YX\ar[r]_-{p_1}\ar[d]^{p_2} & X\ar[d]^f\\
&& X\ar[r]_f & Y
}\]
Ist $x\in X$, so ist $\kappa(x)\cong\kappa(\Delta(x))$, wegen:
\[\xymatrix{
X\ar[rr]^-\Delta\ar[rrd]_{\operatorname{id}} & & X\times_Y X\ar[d]^{p_i}\\
&& X
} \]
Betrachte nun das folgende Diagramm:
\[\xymatrix{
\operatorname{Spec}(K)\ar[rd]_{hi}\ar@{-->}[r]^-g & X\ar[d]^\Delta \\
& X\times_YX 
}\]
...
\[\xymatrix{
U \ar@{-->}[dr]_\theta\ar@/^1pc/[rrd]^u\ar@/_1pc/[ddr]_i &&\\
& X_T\ar[r]_-{p_1}\ar[d]^{f'} & X\ar[d]^f\\
& T\ar[r]_t & Y
} \]
...
\[ \xymatrix{
Z\ar@{^{(}->}[r]\ar[dr] & X'\ar[r]\ar[d]^{f'} & X\ar[d]^f\\
& Y'\ar[r] & Y
} \]
...
\[\xymatrix{
U\ar[r] \ar[d]_i & Z\ar@{^{(}->}[r] \ar[dr] & X'\ar[r]\ar[d]^{f'} & X\ar[d]^f\\
T\ar[rr]\ar@{-->}[rrru]\ar@{-->}[rru] & & Y'\ar[r] & Y
} \]
...
\[\xymatrix{
U\ar[r]^u\ar[d]_i & X\ar[d]\\
T\ar[r]_-t & \operatorname{Spec}(\mathbb{Z})
}\]
...
\[\xymatrix{
\mathbb{Z}\big[\frac{x_0}{x_k},\ldots,\frac{x_n}{x_k}\big]\ar[rr]^-\varphi_-{\frac{x_i}{x_k}\mapsto f_{ik} } \ar[d]&& R\ar[d]\\
...
\mathcal{F}(X)\ar[u]\ar[urr]_{\operatorname{res}}
} \]
...
\[\xymatrix{
X\times_Y X\ar[r]^-{p_2}\ar[d]_{p_1} & X\ar[d]^f\\
X\ar[r]_f & Y
} \]
Nach \hyperref[3.20]{3.20} ist $p_1^{-1}(U)\cap p_2^{-1}(V)=U\times_YV$ affin. Es ist $U\cap V\cong \Delta_{X/Y}(X)\cap U\times_YV\subset U\times_YV$. Nun ist $\Delta_{X/Y}(X)$ abgeschlossen in $X\times_YX$, d.h. $U\cap V$ ist ein abgeschlossenes Unterschema in $U\times_YV$.\qed
...
\[\xymatrix{
\mathbf{P}^r_Y \ar@{=}[r] & Y\times \mathbf{P}_\mathbb{Z}^r \ar[r]\ar[d]& Y\ar[d]\\
