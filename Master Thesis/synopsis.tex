...
Thus, the Hochschild-Serre spectral sequence degenerates on $E_2$:
\[E_2^{i,j}=\mathrm{H}^i(G_K/T, \mathrm{H}^j(T, \mathbb{Z}/\ell\mathbb{Z}(2)))\Longrightarrow\mathrm{H}^{i+j}(G_K, \mathbb{Z}/\ell\mathbb{Z}(2))\]
It yields an isomorphism $\mathrm{H}^2(G_K/T, \mathrm{H}^1(T, \mathbb{Z}/\ell\mathbb{Z}(2)))\cong \mathrm{H}^3(G_K, \mathbb{Z}/\ell\mathbb{Z}(2))$. By Theorem \ref{theorem-absoluteinertia}, we have  $G_K/T\cong G_k$. Since $\mu_{\ell}$ is contained in the absolute inertia field of $K$, $T$ acts trivially on $\mu_{\ell}$, hence:
