...
This section we recall facts from valuation theory.
...
Using the bijection $\operatorname{Spv}_\text{d}^{(2)}(K)\to \operatorname{Spv}_\text{d}^{(2)}(K')$ above we want to establish a bijection $\operatorname{Spv}_\text{rd}^{(2)}(K)\to \operatorname{Spv}_\text{rd}^{(2)}(K')$ in order to establish a bijection between the defectless rank-$1$ valuations:
...
In this section we shall show that $\sigma: G_K\to G_{K'}$ induces group isomorphisms $K'^\times/n\to K^\times/n$ for all $n\in\mathbb{N}$.
...
First of all, we want to show that $\sigma$ induces an isomorphism between the absolute Galois groups of the constant fields $G_k$ and $G_{k'}$. We need the following Lemma:
...
\paragraph{} Let $n\in\mathbb{N}$. In the previous remark, we have replaced $\sigma$, so that the following diagram commutes now:
\[ \begin{tikzcd}
