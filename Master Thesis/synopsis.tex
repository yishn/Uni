...
We write $K_v$ for a henselization of $K$ with respect to $v$.
\end{definition}
...
Thus $v$ is a closed point if and only if it has no non-trivial specializations. Furthermore, for a fixed $v\in \operatorname{Spv}(K)$ there is a canonical bijection: 
\[\{ w\in\operatorname{Spv}(K)\mid \mathcal{O}_w\subset\mathcal{O}_v \} \to \operatorname{Spv}k(v),\ w\mapsto w\mod v\]
Therefore $v$ is a closed point iff $k(v)$ only admits the trivial valuation, i.e. $k(v)$ has positive characteristic $p>0$ and is algebraic over $\mathbb{F}_p$. In other words, $v$ is a closed point iff $\dim k(v)=0$. By the dimension inequality \ref{rank-dimension-inequality} we have $\operatorname{Spv}^{(n)}(K)\subset |\operatorname{Spv}(K)|$ where $n=\dim(K)$.
...
\begin{proof}
See Valued Fields 4.1.4.
\end{proof}

\begin{theorem}\label{theorem-maxunramified}
Let $K$ be a henselian field with respect to $v: K\to\Gamma\cup\{\infty\}$. Let $K^\text{nr}$ be the maximal unramified extension of $K$ with the unique extension $w$ of $v$. Then:
\begin{enumerate}[(i)]
\item $w$ has value group $\Gamma$ and residue class field $k(v)^\text{s}$.
\item $K^\text{nr}$ is a Galois extension of $K$ and $\operatorname{Gal}(K^\text{nr}/K)\cong G_{k(v)}$. There exists an inclusion-preserving one-to-one correspondence: 
\begin{align*}
\{ \text{Intermediate extensions in $K^\text{nr}/K$} \} &\longrightarrow \{\text{Separable extensions of $k(v)$} \}\\ 
L &\longmapsto k(v_L)  
\end{align*}
...
Let $v_1,v_2\in\operatorname{Spv}(K)$ be two inequivalent valuations of rank $1$ on a field $K$ with decomposition groups $G_{v_1},G_{v_2}\subset G_K$. Then $G_{v_1}\cap G_{v_2}=1$.
\end{corollary}

\begin{proof}
Let $K_{v_i}$ be a henselization of $K$ with respect to $v_i$. The compositum $K_{v_1}K_{v_2}$ in a fixed separable closure of $K$ is henselian with respect to both $v_1$ and $v_2$, hence separably closed by Theorem \ref{theorem-fkschmidt}. Therefore:
\[1=G_{K_{v_1}K_{v_2}} = G_{K_{v_1}}\cap G_{K_{v_2}} = G_{v_1}\cap G_{v_2} \qedhere\]
\end{proof}

\section{The Main Lemma}

Let $K$ be a (not necessarily finitely generated) field over $\mathbb{Q}$ and dimension $2$.
...
\end{enumerate}
In this case, $k(v')$ is separably closed where $v'$ is the unique extension of the valuation on $K_v$ to $K_{w_1}K_{w_2}$ where $v=v(w_1)$. By Theorem \ref{theorem-maxunramified} $K_{w_1}K_{w_2}$ is the maximal unramified field extension $K_v^\text{nr}$ of $K_v$. Furthermore, $K'_{\sigma(w_1)}K'_{\sigma(w_2)} = (K_{w_1}K_{w_2})' \neq \overline{K'}$ and again by Lemma \ref{2.2} we get: \[\sigma(w_1),\sigma(w_2)\in\operatorname{Spv}_\text{rd}^{(2)}(K)\quad \text{and} \quad v(\sigma(w_1)) = v(\sigma(w_2))\]
...
Therefore, $\sigma$ induces with Theorem \ref{theorem-maxunramified} the following isomorphism:
...
\begin{proof}
\begin{enumerate}[(i)]
\item Consider the maximal unramified extension $L = K_P^\text{nr}$ of $K_P$ and let $w$ be the unique extension of $v_P$ in $L$. Now $k(w) = k(v_P)^\text{s}$. Since $w$ is unramified over $v$, $w$ is discrete as well. We have the following exact sequence:
...
Therefore $\sigma(g)^{-1}\tau(g)$ is contained in $G_{\overline{v}}$ for all $\overline{v}\in\operatorname{Spv}_\text{d}^{(1)}(\overline{K})$. By the Theorem of F.K. Schmidt \ref{corollary-fkschmidt} we have $G_{\overline{v}_1}\cap G_{\overline{v}_2} = 1$ for $\overline{v}_1\neq\overline{v}_2$. Therefore, $\sigma(g) = \tau(g) = \varphi^{-1}g\varphi$ for all $g\in G_K$.
