...
\item \textit{Case 2:} Let $x + y\neq 0$. Looking at the commutative diagram in Step 1 we get
\begin{align*}
r_P(\varphi(x+ y)) &= \varphi_P(r_{P'}(x + y)) \\
&= \varphi_P(r_{P'}(x)) + \varphi_P(r_{P'}(y)) = r_P(\varphi(x) + \varphi(y))
\end{align*}
for almost all $P\in X_0$. Note that $\varphi_P: k(P')\to k(P)$ is a field isomorphism and that the residue maps $r_{P}: \mathcal{O}_{P} \to k(P)$ are ring homomorphisms. Therefore, $\varphi(x+y) = \varphi(x) + \varphi(y)$.\qedhere
\end{itemize}
\end{proof}
