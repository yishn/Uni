...
\end{tikzcd} \]
We have $\operatorname{cor}\circ\operatorname{res} = [L:K]$. Since $[L:K]$ is coprime to $\ell$, we see that $\operatorname{res}$ is injective. Thus we can assume that $K=L$, i.e. $\mu_{\ell}\subset K$ and $K$ is not formally real. Now we can replace $\mathbb{Z}/\ell\mathbb{Z}$ by $\mathbb{Z}/\ell\mathbb{Z}(2)$ and let us write $\mathrm{H}^3(L) = \mathrm{H}^3(L, \mathbb{Z}/\ell\mathbb{Z}(2))$. 
