...
\end{align*}
Valuations in $\operatorname{Spv}_\text{d}(K)$ are called \textit{defectless}\index{valuation!defectless}, whereas those in $\operatorname{Spv}_\text{rd}(K)$ are called \textit{rank-defectless}\index{valuation!rank-defectless}. For $\ast\in\{\text{d},\text{rd}\}$ we set:
...
$\operatorname{Spv}(K)$ with the topology defined above is a quasicompact $T_0$-space. In particular the set of closed points $|\operatorname{Spv}(K)|$ of $\operatorname{Spv}(K)$ is a quasicompact $T_1$-space, endowed with the induced topology. Let $v,w\in\operatorname{Spv}(K)$. We have:
...
\begin{proof}
See [ZS] VI §17 Theorem 38, §17 Theorem 40.
\end{proof}
...
\end{align*}
Therefore $v$ is a closed point iff $k(v)$ only admits the trivial valuation, i.e. $k(v)$ has characteristic $p>0$ and is algebraic over $\mathbb{F}_p$. In other words, $v$ is a closed point iff $\dim k(v)=0$. By the dimension inequality \ref{rank-dimension-inequality} we have $\operatorname{Spv}^{(n)}(K)\subset |\operatorname{Spv}(K)|$ where $n=\dim(K)$.
...
\begin{proof}
See Valued Fields 4.1.4, [ZS] VI §3 Theorem 3.
\end{proof}

\begin{theorem}\label{theorem-henselization}
Let $K$ be a field and $v\in\operatorname{Spv}(K)$. Then there is a Galois extension $K_v/K$ with a henselian extension $\overline{v}$ of $v$ with the following characterization:
...
Furthermore, $\overline{v}$ has the same value group as $v$ and $k(\overline{v}) = k(v)$. $G_{K_v}$ is the decomposition group $G_{\overline{v}}\subset G_K$ of the unique extension of $\overline{v}$ to $K^\text{s}$. $K_v$ is called a \textit{henselization} of $K$ with respect to $v$. Any two henselization of $K$ with respect to $v$ are $K$-conjugate.
\end{theorem}

\begin{proof}
See Valued Fields Lemma 5.2.1, Lemma 5.2.2, Theorem 5.2.5.
\end{proof}

\begin{theorem}\label{theorem-absoluteinertia}
Let $K$ be a henselian field with respect to $v: K\to\Gamma\cup\{\infty\}$. Let $K^\text{t}$ be the absolute inertia field of $K$ with the unique extension $w$ of $v$. Then:
\begin{enumerate}[(i)]
\item $w$ has value group $\Gamma$ and residue class field $k(v)^\text{s}$.
\item $K^\text{t}$ is a Galois extension of $K$ and $\operatorname{Gal}(K^\text{t}/K)\cong G_{k(v)}$. There exists an inclusion-preserving one-to-one correspondence: 
\begin{align*}
...
\begin{proof}
Let $L$ be the fixed field of $G_{w_1}\cap G_{w_2}$ and set $v_i = w_i|_L$. Then $L$ is henselian with respect to both $v_1$ and $v_2$. Since $v_1$ and $v_2$ are unramified in $K^\text{s}/L$, they are inequivalent valuations of rank $1$. Now $L$ is separably closed by Theorem \ref{theorem-fkschmidt}. Therefore by Theorem \ref{theorem-henselization} we get $G_{{w_1}}\cap G_{{w_1}}=G_L =1$.
\end{proof}
...
\begin{proof}
Consider the extension $L=K(\mu_{\ell},\sqrt{-1})/K$. We have the restriction and corestriction map:
\[ \operatorname{res}: \mathrm{H}^3(K,\mathbb{Z}/\ell\mathbb{Z}) \to \mathrm{H}^3(L,\mathbb{Z}/\ell\mathbb{Z}) \qquad \operatorname{cor}:\mathrm{H}^3(L,\mathbb{Z}/\ell\mathbb{Z})\to \mathrm{H}^3(K,\mathbb{Z}/\ell\mathbb{Z}) \]
We have $\operatorname{cor}\circ\operatorname{res} = [L:K]$. Since $[L:K]$ is coprime to $\ell$, we see that $\operatorname{res}$ is injective. The same argument can be made for $K_w(\mu_{\ell},\sqrt{-1})/K_w$. Thus we can assume that $K=L$, i.e. $\mu_{\ell}\subset K$ and $K$ is not formally real. Now we can replace $\mathbb{Z}/\ell\mathbb{Z}$ by $\mathbb{Z}/\ell\mathbb{Z}(2) = \mu_\ell^{\otimes 2}$ and let us write $\mathrm{H}^3(L) = \mathrm{H}^3(L, \mathbb{Z}/\ell\mathbb{Z}(2))$. 
...
Now the first assertion follows from [Se], Ch. I, §4.
...
for some $d \leq \infty$, where $\Gamma'$ denotes the value group and $k'$ the residue field of the unique prolongation of $w$ to $L$. Note that $\operatorname{rank}(w) = \operatorname{rank}(v)$ in an algebraic field extension ([ZS], Ch. VI, §11, Lemma 1 and 2).
\end{proof}
...
\end{enumerate}
In this case, $k(v')$ is separably closed where $v'$ is the unique extension of the valuation on $K_v$ to $K_{w_1}K_{w_2}$ where $v=v(w_1)$. By Theorem \ref{theorem-absoluteinertia} (ii) $K_{w_1}K_{w_2}$ is the absolute inertia field $K_v^\text{t}$ of $K_v$. Furthermore, $K'_{\sigma(w_1)}K'_{\sigma(w_2)} = (K_{w_1}K_{w_2})' \neq \overline{K'}$ and again by Lemma \ref{2.2} we get: \[\sigma(w_1),\sigma(w_2)\in\operatorname{Spv}_\text{rd}^{(2)}(K)\quad \text{and} \quad v(\sigma(w_1)) = v(\sigma(w_2))\]
...
\end{tikzcd} \]
Let us consider the morphism $f:X'\times_{k'} k\to X'$. We want to see that $f$ splits completely. For $P'\in X'_0$ we have as topological spaces:
...
\end{tikzcd} \]
where the second map is induced by the isomorphism $\overline{K''}\to\overline{K'},\ x\otimes y\mapsto \varphi^{-1}(y)\cdot x$, we may assume that $k=k'$, $\overline{k} = \overline{k'}$ and that $\varphi:\overline{k}\to\overline{k}$, $\sigma: G_k\to G_k$ are identities.
\end{remark}

\paragraph{} Let $n\in\mathbb{N}$. In the previous remark, we have reduced the problem, so that the following diagram commutes: 
\[ \begin{tikzcd}
...
\begin{proof}
\begin{enumerate}[(i)]
\item Consider the absolute inertia field $L = K_P^\text{t}$ of $K_P$ and let $w$ be the unique extension of $v_P$ in $L$. By Theorem \ref{theorem-absoluteinertia} (i), $w$ is discrete as well with $k(w) = k(v_P)^\text{s}$. We have the following exact sequence:
...
\item Consider the field extension $K_P/K$. Without loss of generality, we can assume $K=K_P$. Since $\operatorname{char}k(P)=0$, by Hensel's lemma, $r_P/n$ is an isomorphism and the composition
\[\begin{tikzcd}
...
where $\operatorname{NS}(C)$ denotes the finitely generated Néron-Severi group of $C$. Now $\operatorname{Pic}^0(C)$ can be identified with the $F$-rational points of the Jacobian variety $J$ of $C$. $J(F)$ is finitely generated by the Mordell-Weil Theorem \ref{mordell-weil-theorem}, hence $\operatorname{Pic}(C)$ is finitely generated as well.
\end{proof}
...
Such a set $S$ is called \textit{admissible}. If $T\subset X_0$ is a finite set of closed points, then we can choose $S$ in $X_0\setminus T$.
...
is admissible. Now let $T\subset X_0$ be finite. By the Weak Approximation Theorem \ref{weak-approximation}, we can add a principal divisor to the $D_i$ and the $D'_j$ so that their support is in $X_0\setminus T$ and $X'_0\setminus T'$ respectively.
\item Observe that the following sequence is exact:
\[ \begin{tikzcd}
1 \ar[r] & K^S \ar[r]& K^\times \ar[r, "\operatorname{div}"]& \displaystyle \bigoplus_{P\in X_0\setminus S}\mathbb{Z} \ar[r]& \operatorname{Pic}(X\setminus S) = 0
\end{tikzcd} \]
...
\[ K^S/n = \bigcap_{P\in X_0\setminus S}U_P/n \]
...
\end{align*}
for all $P\in X_0\setminus S$. Note that $\varphi_P: k(P')\to k(P)$ is a field isomorphism and that the residue maps $r_{P'}: \mathcal{O}_{P'} \to k(P')$ are ring homomorphisms. Since $K^S\to \prod_{P\in X_0\setminus S} k(P)^\times$ is injective, we get $\varphi(x+y) = \varphi(x) + \varphi(y)$.\qedhere
\end{itemize}
\end{proof}
