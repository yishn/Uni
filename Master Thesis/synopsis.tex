...
\[ 1\longrightarrow \mu_n\longrightarrow (F^\text{s})^\times \stackrel{n}{\longrightarrow} (F^\text{s})^\times \longrightarrow 1 \]
This gives rise to the following long exact cohomology sequence:
\[\begin{tikzcd}
\cdots\longrightarrow F^\times \ar[r, "n"]& F^\times \ar[r]& \mathrm{H}^1(F, \mu_n)\ar[r] & \mathrm{H}^1(F, (F^\text{s})^\times)\ar[dlll]\\
\mathrm{H}^1(F, (F^\text{s})^\times)\ar[r] & \mathrm{H}^2(F, \mu_n)\ar[r] & \operatorname{Br}(F)\ar[r, "n"] & \operatorname{Br}(F) \longrightarrow \cdots
\end{tikzcd}\]
