...
\item $\operatorname{Spv}^{(1)}(K) = \operatorname{Spv}^{(1)}_\text{d}(K)$
\end{enumerate}
\end{lemma}

\begin{proof}
\begin{enumerate}[(i)]
\item Let $v\in\operatorname{Spv}^{(2)}(K)\subset |\operatorname{Spv}(K)|$. By Lemma \ref{2.2-prepre} we have $\dim k(v) = 0$ and thus the equality of the dimension inequality in Lemma \ref{rank-dimension-inequality}. By Lemma \ref{dimension-equality} $\Gamma_v$ is a finitely generated abelian group of rank $2$, thus not a subgroup of $\mathbb{R}$, i.e. $\operatorname{rank}(v)> 1$. 
\item Let $v\in\operatorname{Spv}^{(1)}(K)$. If $\operatorname{rank}(v) = 0$, $v$ would be the trivial valuation with trivial value group $\Gamma_v = 0$ and rational rank $0$. \qedhere
\end{enumerate}
\end{proof}
