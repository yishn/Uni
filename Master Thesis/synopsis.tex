...
\item Assume that (i) is satisfied for two different prime numbers. Then $w$ is defectless.
\end{enumerate}
\end{proposition}
...
\item Choose the prime $\ell$ different from $\operatorname{char} k(w)$. According to Lemma \ref{2.5} we have $\operatorname{rrank}(w)=\dim_\mathbb{Q}(\Gamma_w\otimes_\mathbb{Z}\mathbb{Q} )\geq \dim_{\mathbb{F}_\ell}(\Gamma_w /\ell\Gamma_w) = 2 = \dim(K)$.
\end{enumerate}
\end{proof}
...
Thus $\sigma: \operatorname{Spv}_\text{d}^{(2)}(K)\to \operatorname{Spv}_\text{d}^{(2)}(K')$ maps $\operatorname{Spv}_\text{rd}^{(2)}(K)$ onto $\operatorname{Spv}_\text{rd}^{(2)}(K')$. Furthermore:
...
\begin{proof}
\begin{enumerate}[(i)]
\item  We have $(K^\text{t}_P)^\times/n\cong \mathbb{Z}/n\mathbb{Z}$ since $\operatorname{char}k(P) = 0$. The canonical map $K^\times\to (K^\text{t}_P)^\times/n$ can be identified with the map $K^\times /n\to\mathbb{Z}/n$ induced by $v_P$. Now the following commutative diagram gives us the result:
\[ \begin{tikzcd}
0 \ar[d] & 0 \ar[d]\\
...
\end{tikzcd} \]
Since the rightmost module is $\mathbb{Z}$-flat, tensoring with $\mathbb{Z}/n$ yields another exact sequence and we can consider $K^S/n$ as a subgroup of $K^\times/n$. Then:
...
An abelian group is called \textit{free-by-finite} if its torsion subgroup $A_\text{tor}$ is finite and the quotient $A/A_\text{tor}$ is free abelian. $A$ is free-by-finite if and only if $A$ is the direct sum of a finite group and a free abelian group.
\end{definition}
...
\item Let $0\longrightarrow A'\stackrel{i}{\longrightarrow}A\stackrel{p}{\longrightarrow}A''\longrightarrow 0$ be an exact sequence of abelian groups. If $A'$ and $A''$ are free-by-finite, then $A$ is free-by-finite. Conversely, if $A$ is free-by-finite, then $A'$ is free-by-finite. If additionally, $A'$ is finitely generated, then $A''$ is free-by-finite.
...
\begin{proof}
\begin{enumerate}[(i)]
\item Let $A$ be free-by-finite, i.e. $A = A' \oplus F$ for some finite group $A'$ and free group $F$. Since $\ker(f)\subset A$ is finite, it's actually contained in $A'$.

Now let $B$ be free-by-finite and $K = \ker(f)$. Since the injective map $A/K \to B$ induced by $f$ can be restricted to their torsion subgroups, we see that $(A/K)_\text{tor}$ is finite. Every $a\in A_\text{tor}$ can be written as $a = a' + k$ for some $a'\in A,\ k\in K$ such that $na'\in K$ for some $n\in\mathbb{N}$. Since both $(A/K)_\text{tor}$ and $K$ are finite, $A_\text{tor}$ is finite as well. TODO
\item TODO
\item It suffices to show this assertion for finite groups and for $A=\mathbb{Z}$. For $A$ finite we have $nA = 0$ for $n = \#A$. Since $\mathbb{Z}$ has no nilpotent elements, the intersection of all prime ideals of $\mathbb{Z}$ is already trivial. \qedhere
\end{enumerate}
\end{proof}
...
\[ \begin{tikzcd}
1 \ar[r] & \displaystyle \prod_{i=1}^{n-1} F_i^\times \ar[r]& \displaystyle \Big(\prod_{i=1}^{n}F_i^\times\Big)/F^\times \ar[r]& F_n^\times/F^\times \ar[r]& 1
\end{tikzcd} \]
The left group is free-by-finite by (i), and by Lemma \ref{5.2} (ii), we can use induction to reduce to the case $n=1$.
...
where $f:\operatorname{Cl}(F)\to \operatorname{Cl}(F_1)$. Since the ideal class groups are finite, according to Lemma \ref{5.2} (i), it suffices to show that $\mathcal{I}(F_1)/\mathcal{I}(F)$ is free-by-finite. This follows from the fact that there are only finitely many primes of $F$ which are ramified in $F_1$.\qedhere
\end{enumerate}
\end{proof}
