...
\begin{proof}
See \cite{EP05} Section 2.3, Corollary 4.1.4, \cite{ZS60} VI, §3 Theorem 3.
\end{proof}
...
\begin{proof}
(i) and (ii) follow from \cite{HK94} Lemma 2.4.3, and (iii) is a consequence of (ii) and the fact that henselizations commute with filtrated direct limits, e.g. see \cite{EGA4} Proposition 18.6.14.
\end{proof}
...
Let $K$ be henselian with respect to a discrete valuation $v$, $k=k(v)$ its residue field with $\operatorname{cd}(k)\leq 2$. Let $\ell\neq p$ be a prime where $p=\operatorname{char}(k)$. Then:
...
Inspection of the Hochschild-Serre spectral sequence 
\[E_2^{i,j}=\mathrm{H}^i(G_K/T, \mathrm{H}^j(T, \mathbb{Z}/\ell\mathbb{Z}(2)))\Longrightarrow\mathrm{H}^{i+j}(G_K, \mathbb{Z}/\ell\mathbb{Z}(2))\]
yields an isomorphism $\mathrm{H}^2(G_K/T, \mathrm{H}^1(T, \mathbb{Z}/\ell\mathbb{Z}(2)))\cong \mathrm{H}^3(G_K, \mathbb{Z}/\ell\mathbb{Z}(2))$. By Theorem \ref{theorem-absoluteinertia}, we have  $G_K/T\cong G_k$. Since $\mu_{\ell}$ is contained in the absolute inertia field of $K$, $T$ acts trivially on $\mu_{\ell}$, hence:
...
By Theorem \ref{theorem-sylowgroups-inertiagroup}, $T/V$ is the free pro-$\mathcal{C}$ group, where $\mathcal{C}$ is the class of all finite groups with order prime to $p$. Therefore $\operatorname{Hom}_\text{cont.}(T/V, \mu_{\ell})\cong \mu_{\ell}$. The rest follows from Proposition \ref{prop:kummer-theory}.
\end{proof}
...
Let $v$ be a discrete valuation of $K$ with residue field $k=k(v)$. By Lemma \ref{2.4-pre}, the map
...
This map is injective by the classical Hasse principle and its image is contained in $\bigoplus {}_\ell\operatorname{Br}(k_{\widetilde{w}})$. Combining these maps yields the injection:
...
Now we calculate the cohomology groups $\mathrm{H}^3(K_w,\mathbb{Z}/\ell\mathbb{Z})$ for $w\in |\operatorname{Spv}(K)|$. With the help of our Main Lemma \ref{2.4} we can find a cohomological criterion of henselian fields over $K$ with respect to a $w\in |\operatorname{Spv}(K)|$.
...
\end{cases} \] 
since $\operatorname{cd}(\mathbb{Z}_\ell)=1$. Thus $\mathrm{H}^3(G_\ell, \mathbb{Z}/\ell\mathbb{Z}) =0$ iff $G_k(\ell) = 0$ or $r < 2$. Now let $G_k(\ell) \cong\mathbb{Z}_\ell$ and $r = 2$. Inspecting the Hochschild-Serre spectral sequence yields an isomorphism:
...
For the second assertion, we assume $\ell^\infty\nmid [k:\mathbb{F}_p]$ and $\dim_{\mathbb{F}_\ell}(\Gamma / \ell\Gamma) = 2$. By \cite{En72} Theorem 20.21, we have the following equation of supernatural numbers:
\[ [L:K] = (\Gamma':\Gamma) \cdot [k':k]\cdot p^d \]
for some $d \leq \infty$, where $\Gamma'$ denotes the value group and $k'$ the residue field of the unique extension of $w$ to $L$. By using this equation and the already proven statement from the first part, it suffices to show the following equivalences:
\begin{enumerate}[(i)]
...
Let $n\in\mathbb{N}$. In this section we shall show that $\sigma: G_K\to G_{K'}$ induces group isomorphisms $\varphi_n: K'^\times/n\to K^\times/n$. Due to Proposition \ref{prop:kummer-theory}, we have isomorphisms:
