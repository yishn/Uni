...
Let $S\subset X_0$ be admissible and $P_0\in X_0\setminus S$. Then there exists a finite subset $T\subset X_0\setminus S$ with $P_0\in T$ such that the following map is injective:
...
Hence, $\varphi^{S_1}=\varphi^{S_2}|_{K'^{S'_1}}$. Thus, we can combine the isomorphisms $\varphi^S$, where $S$ ranges over the admissible subsets of $X_0$, into an isomorphism $\varphi = \varinjlim \varphi^S: K'^\times \to K^\times$. Note that for every $x\in K^\times$ we have $v_P(x)= 0$ for almost all $P\in X_0$; similarly for $K'$ and $X'_0$.

Now note that for any $x\in U_{P'}$ we can find an admissible set $S\subset X_0\setminus \{P\}$ such that $x\in K'^{S'}$.
\end{proof}
...
\begin{proof}
Let $\tau:G_K\to G_{K'},\ g\mapsto \varphi^{-1}\circ g\circ \varphi$. Clearly, $\tau$ is a group isomorphism and we have $\tau(G_L)=G_{L'}=\sigma(G_L)$ for every finite subextension $L/K$ of $\overline{K}/K$. By Lemma \ref{3.2}, $\tau$ induces bijections $\tau: \operatorname{Spv}_\text{d}^{(1)}(L)\to \operatorname{Spv}_\text{d}^{(1)}(L')$ for any intermediate field $L$ of $\overline{K}/K$.
\end{proof}