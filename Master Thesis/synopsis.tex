...
If $w\in\operatorname{Spv}_\text{rd}^{(i)}(K)$ for some $i\geq 1$ then there exists a unique $v(w)\in\operatorname{Spv}_\text{d}^{(i-1)}(K)$ such that $\mathcal{O}_w\subset\mathcal{O}_{v(w)}$. $w$ is henselian if and only if $v(w)$ and $w\mod v(w)$ are henselian.
\end{lemma}

\begin{proof}
See Valued Fields 2.3, 4.1.4, [ZS] VI §3 Theorem 3.
\end{proof}
...
$K_v$ is called a \textit{henselization} of $K$ with respect to $v$. Furthermore, $\overline{v}$ has the same value group as $v$ and the same residue field $k(\overline{v}) = k(v)$. $G_{K_v}$ is the decomposition group $G_{\overline{v}}\subset G_K$ of the unique extension of $\overline{v}$ to $K^\text{s}$. 
...
\begin{proof}
Let $\overline{w}$ be the extension of $w\in\operatorname{Spv}(K_w)$ on $K^\text{s}$. By Valued Fields 3.1.5 we can find an extension $\overline{v}$ of $v$ on $K^\text{s}$ such that $\overline{w}$ is a specialization of $\overline{v}$. Let $K_v$ be the fixed field of $G_{\overline{v}}\subset G_K$.

Let $\sigma\in G_{\overline{w}}$. Then both $\mathcal{O}_{\overline{v}}$ and $\sigma(\mathcal{O}_{\overline{v}})$ are overrings of $\mathcal{O}_{\overline{w}}$. Now the overrings of valuation rings are totally ordered by inclusion and correspond to localizations of $\mathcal{O}_{\overline{w}}$ at prime ideals (See Valued Fields 2.3). Since $\mathcal{O}_{\overline{v}}$ and $\sigma(\mathcal{O}_{\overline{v}})$ have the same Krull dimension, they must be the same, thus $\sigma\in G_{\overline{v}}$. This proves $K_v\subset K_w$.
...
for some $d \leq \infty$, where $\Gamma'$ denotes the value group and $k'$ the residue field of the unique prolongation of $w$ to $L$. Note that $\operatorname{rank}(w) = \operatorname{rank}(v)$ in an algebraic field extension ([ZS], Ch. VI, §11, Lemma 2).
\end{proof}
...
\begin{definition}
For each admissible $S\subset X_0$ and $T_1\subset X_0\setminus S$ with injective $r_{S,T_1}$ we also have the injectivity of $r_{S,T_2}$ for every $T_2 \supset T_1$. Therefore,  we can find a finite subset $T\subset X_0\setminus S$ such that both $r_{S,T}$ and $r_{S',T'}$ are injective. Such a $T$ will be called $S$-\textit{admissible} and $(S,T)$ is called \textit{admissible pair}.
\end{definition}
