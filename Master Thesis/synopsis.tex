...
Since $K$ is assumed to be finite dimensional, Lemma \ref{rank-dimension-inequality} ensures that this sequence ends with a closed point $v_n\in \overline{\{v\}}\cap U_S\subset A\cap U_S$.
...
The following lemma is the key result in proving Theorem \ref{thm:main-result}. First, we use K. Kato's two-dimensional local-global principle to show the assertion for finitely generated $K$. Then we'll use a quasi-compactness argument to drop the assumption that $K$ is finitely generated.
...
\begin{proof}
Just like in Lemma \ref{2.4-finitelygenerated} we can assume that $\mu_\ell\subset K$ and $K$ is not formally real. Furthermore, we replace $\mathbb{Z}/\ell\mathbb{Z}$ by $\mathbb{Z}/\ell\mathbb{Z}(2)$. We adopt the notation from the previous proofs.
...
In this chapter we want to see that $\sigma$ induces bijections on defectless valuations. A direct consequence of Proposition \ref{2.6} is the bijection $\operatorname{Spv}_\text{d}^{(2)}(K)\to \operatorname{Spv}_\text{d}^{(2)}(K')$:
...
\end{enumerate}
The implication (i)$\implies$(ii) follows from Lemma \ref{2.2}. Now assume (ii) holds and let $v=v(w_1)\in\operatorname{Spv}^{(1)}_\text{d}(K)$. We have $K_{w_1}K_{w_2}\subset K_v^\text{t}$. By Theorem \ref{theorem-absoluteinertia}, the value group of $v$ on $K_v^\text{t}$ is equal to the value group of $v$ on $K$, which is finitely generated by Lemma \ref{rank-dimension-inequality}. However, value groups of valuations on an algebraically closed field are divisible and in particular not finitely generated, hence $K_v^\text{t}$ can't be algebraically closed and neither can the composite $K_{w_1}K_{w_2}$. 

Furthermore, if (ii) holds, the extension of $v$ on $K_{w_1}K_{w_2}$ has a separably closed residue field. By Theorem \ref{theorem-absoluteinertia}, we have $K_{w_1}K_{w_2}=K_v^\text{t}$.
...
\begin{lemma}\label{lemma-sigma-groupaction-valuation}
...
\section{Isomorphism of Constant Fields}

In this section we shall establish an isomorphism between the absolute Galois groups of the constant fields $k$ and $k'$. In particular, since both $k$ and $k'$ are number fields, the theorem of Neukirch-Uchida \ref{thm:neukirch-uchida} will yield an isomorphism between $k$ and $k'$.
...
We have $(K\overline{k})'=K'\overline{k'}$, i.e. $\sigma(G_{K\overline{k}})=G_{K'\overline{k'}}$. $\sigma$ induces an isomorphism $\sigma: G_k\to G_{k'}$ such that the following diagram commutes:
\[ \begin{tikzcd}
...
Everything else follows from the theorem of Neukirch-Uchida \ref{thm:neukirch-uchida}.
\end{proof}
...
Our goal is to establish an isomorphism between these cohomology groups.
...
The operation of $G_K$ and $G_{K'}$ on $\mu_n$ factors through $G_k$, thus we have $\sigma(g)\zeta = g\zeta$ for all $\zeta\in\mu_n$ and $g\in G_K$. Therefore, for all intermediate fields $L$ of $\overline{K}/K$, $\sigma$ induces an isomorphism:
...
\chapter{Proof of Pop's Theorem}

Before we start proving Pop's Theorem \ref{thm:main-result}, we need some tools that will help us construct a group isomorphism $\varphi: K'^\times\to K^\times$ from the modulo $n$ isomorphisms $\varphi_n: K'^\times/n\to K^\times/n$ which we constructed in the previous chapter.
...
Let $C$ be a smooth projective curve over a number field $F$. Then its Picard group $\operatorname{Pic}(C)$ is finitely generated.
\end{corollary}
...
\end{align*}
Note that $\varphi_P: k(P')\to k(P)$ is a field isomorphism and that the residue maps $r_{P'}: \mathcal{O}_{P'} \to k(P')$ are ring homomorphisms. Since $K^S\to \prod_{P\in X_0 -  S} k(P)^\times$ is injective by Lemma \ref{5.4}, we get $\varphi(x+y) = \varphi(x) + \varphi(y)$.\qedhere
\end{itemize}
\end{proof}
