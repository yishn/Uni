...
\begin{lemma}\label{dimension-equality}
...
Let $K$ be finitely generated, $\ell\neq 2$ be a prime and $f\in\mathrm{H}^3(K, \mathbb{Z}/\ell\mathbb{Z})$. Then the support of $f$ is closed in $|\operatorname{Spv}(K)|$:
...
First, let's consider the case where $K$ is finitely generated. Let $\mathfrak{X}$ be a model of $K$. $\mathfrak{X}_i$ denotes the set of points $x\in\mathfrak{X}$ whose closure $\overline{\{x\}}$ is $i$-dimensional. For $x\in\mathfrak{X}_1$ the local ring $\mathcal{O}_{\mathfrak{X}, x}$ is a discrete valuation ring and defines a valuation $v_x\in\operatorname{Spv}_\text{d}^{(1)}(K)$. By [K1] Theorem 0.8, the following map
...
The direct sum ranges over a subset of $|\operatorname{Spv}(K)|$, thus this shows the assertion. Now, we drop the assumption that $K$ is finitely generated.
...
Since $K_j$ is finitely generated and we proved the statement for finitely generated fields, we obtain $\operatorname{res}_{K_j/K_0}(g) = 0$, thus $f = \operatorname{res}_{K/K_0}(g) = 0$.
\end{proof}
